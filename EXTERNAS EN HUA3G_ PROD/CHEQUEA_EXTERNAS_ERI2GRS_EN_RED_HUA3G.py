...
for celda in celdas:
    if celda[9]=='Orange-Ericsson':
        ...
        if externa_valida==1 and def_ext_valida==1:
            ...
            if ci_ext - def_externa_eri_ci <> 0: print 'CI EN DEF EXTERNA: ' + celda[0],str(ci_ext) + '  CI EN RED: ' + externa_eri, def_externa_eri_ci
            if ncc_ext - def_externa_eri_ncc <> 0: print 'NCC EN DEF EXTERNA: ' + celda[0], str(ncc_ext) + '  NCC EN RED: ' + externa_eri, def_externa_eri_ncc
            if bcc_ext - def_externa_eri_bcc <> 0: print 'BCC EN DEF EXTERNA: ' + celda[0], str(bcc_ext) + '  BCC EN RED: ' + externa_eri, def_externa_eri_bcc
            if mcc_ext - def_externa_eri_mcc <> 0: print 'MCC EN DEF EXTERNA: ' + celda[0], str(mcc_ext) + '  MCC EN RED: ' + externa_eri, def_externa_eri_mcc
            if mnc_ext - def_externa_eri_mnc <> 0: print 'MCC EN DEF EXTERNA: ' + celda[0], str(mnc_ext) + '  MNC EN RED: ' + externa_eri, def_externa_eri_mnc
