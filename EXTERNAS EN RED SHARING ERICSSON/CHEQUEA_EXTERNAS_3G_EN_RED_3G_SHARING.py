...
f=open('C:\\CHEQUEO EXTERNAS\\CORRECCIONES_EXT_ERI3G_EN_RED_ERI3G_SHARING.csv','wb')
...
for celda in celdas:
    ...
    if externa_valida == 1 and def_ext_valida == 1:
        ...
        if psc_ext - def_externa_hua_psc <> 0: writer.writerow(['', str(externa_eri), 'PSC', str(psc_ext), str(def_externa_hua_psc)])
        if lac_ext - def_externa_hua_lac <> 0: writer.writerow(['', str(externa_eri), 'LAC', str(lac_ext), str(def_externa_hua_lac)])
        if ci_ext - def_externa_hua_ci <> 0: writer.writerow(['', str(externa_eri), 'CI', str(ci_ext), str(def_externa_hua_ci)])
        if mcc_ext - def_externa_hua_mcc <> 0: writer.writerow(['', str(externa_eri), 'MCC', str(mcc_ext), str(def_externa_hua_mcc)])
        if mnc_ext - def_externa_hua_mnc <> 0: writer.writerow(['', str(externa_eri), 'MNC', str(mnc_ext), str(def_externa_hua_mnc)])
