...
f=open('C:\\CHEQUEO EXTERNAS\\CORRECCIONES_EXT_HUA2G_EN_RED_HUA3G.csv','wb')
...
for celda in celdas:
    if celda[9]=='Huawei':
        ...
        if externa_valida==1 and def_ext_valida==1:
            ...
            if bcch_ext - def_externa_hua_bcch <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'BCCH', str(bcch_ext), str(def_externa_hua_bcch)])
            if lac_ext - def_externa_hua_lac <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'LAC', str(lac_ext), (def_externa_hua_lac)])
            if ci_ext - def_externa_hua_ci <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'CI', str(ci_ext), (def_externa_hua_ci)])
            if ncc_ext - def_externa_hua_ncc <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'NCC', str(ncc_ext), str(def_externa_hua_ncc)])
            if bcc_ext - def_externa_hua_bcc <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'BCC', str(bcc_ext), str(def_externa_hua_bcc)])
            ...
            if bsc_ext <> def_externa_hua_bsc <> 0: writer.writerow([rnc_donde_se_define_ext, str(externa_hua), 'BSC', str(bsc_ext), str(def_externa_hua_bsc)])
